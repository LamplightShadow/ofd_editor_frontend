<template>
  ...
  >
    <p class="merge-hint">
      选择 2～10 个 PDF。先在浏览器下载<strong>原生 PDF</strong>合并结果，再自动导入编辑器（转为 OFD 以便批注与编辑）。
      列表从上到下为合并顺序。
    </p>
...
import { buildMergedFilename, ofdApi, downloadBlob } from '@/api/ofdApi'
...
function onFileSelected(e: Event) {
  ...
  for (const file of files) {
    if (!isPdfFile(file)) {
      ElMessage.warning(`已跳过非 PDF 文件：${file.name}`)
      continue
    }
    if (attachments.value.length >= MAX_FILES) {
      ElMessage.warning(`最多选择 ${MAX_FILES} 个 PDF 文件`)
...
async function handleMerge() {
  ...
  const pdfFilename = buildMergedFilename(files.map((f) => f.name), 'pdf')

  const started = await withUnsavedChangesGuard(DOCUMENT_REPLACE_GUARD_HINT.mergeOpen, async () => {
    merging.value = true
    store.setLoading(true, `正在合并 ${files.length} 个 PDF…`)
    return true
  })
  if (!started) return

  try {
    const pdfBlob = await ofdApi.mergePdf(files)
    ...
    addRecentFile({ name: pdfFile.name, kind: 'pdf' })
